...
class Language
{
    ...
    {
        $langDefault = $this->security->directorySeparator(self::$generalPath . Configuration::$langDefault) . '.ini';
		...
		if (!isset($_COOKIE['lang'])
		    || empty($_COOKIE['lang'])
		    || !file_exists($this->security->directorySeparator(self::$generalPath . $_COOKIE['lang'] . '.ini')))
		{
			setcookie('lang', Configuration::$langDefault, time() + (86400 * 30), "/");
			header("Location: " . $this->security->protocol() . $_SERVER['HTTP_HOST'] . ":" . $_SERVER['SERVER_PORT'] . $_SERVER['REQUEST_URI']);
		}
    }

    public static function getLang($string, $array = 'General')
	{
		$ini = parse_ini_file(Security::directorySeparator(self::$generalPath . $_COOKIE['lang'] . '.ini'), true);
